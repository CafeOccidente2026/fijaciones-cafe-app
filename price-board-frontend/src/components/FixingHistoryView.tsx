...
interface FixingHistoryViewProps {
  ...
  /**
   * When true, skips this component's own Screen chrome (title, back
   * link, filter button in the header) so it can be embedded inside
   * another Screen - e.g. ADMIN's Historial, which toggles between this
   * and PriceHistoryView. The filter toggle still renders, just as a row
   * above the list instead of in the header.
   */
  embedded?: boolean;
}
 ...
 */
export function FixingHistoryView({
  allowUserFilter = false,
  embedded = false,
}: FixingHistoryViewProps) {
  ...
  useEffect(() => {
    CoffeeTypesApi.list(true)
      ...
      .catch(() =>
        setCoffeeTypeOptions([{ label: strings.history.allTypes, value: "" }]),
      );
    ...
    if (allowUserFilter) {
      UsersApi.list("PRODUCER")
        ...
        .catch(() =>
          setUserOptions([{ label: strings.history.allUsers, value: "" }]),
        );
    }
  }, [allowUserFilter]);
  ...
  if (embedded) {
    return (
      <View className="flex-1">
        <View className="mb-3 flex-row justify-end">{filterToggle}</View>
        {list}
      </View>
    );
  }
